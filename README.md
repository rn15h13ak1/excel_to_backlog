...
| `--output-dir DIR` | プレビュー・実行ログの出力先フォルダ（既定: 設定ファイルと同じ場所の `output/`） |
...
`--execute` を指定すると、**新規作成のみ 1 件ずつ内容を表示して可否を確認します**。課題が増えるのは新規作成のときだけなので、そこに確認を集約しています。

**作成・更新の件数は事前に算出しません。** 算出には全行を Backlog と照合する必要があり、行数に比例して「何も起きない待ち時間」が伸びるためです。件数を先に知りたい場合は `--execute` を付けずに実行してください（ドライラン）。

```
  [2] ✅ 更新: DEMO-1 — 既存課題        ← 更新は確認せず実行
...
**既存課題の更新は確認せずに実行します。** 送信内容と既存課題を事前に比較しており、内容が変わらない行は「変更なし」として更新リクエストごと飛ばすため、実際に更新されるのは中身が変わる行だけです。

> 更新は Excel の内容で既存課題を上書きします。Backlog 側で編集した内容がある場合は失われます（`key_col` 方式の場合）。

`-y` / `--yes` を付けると一切確認しません。cron やパイプ経由など**入力を受け取れない環境では `-y` が必須**で、指定がないと終了コード 1 で停止します。

### 元の Excel ファイルは変更しません

このツールは読み込んだ Excel を一切書き換えません。生成するのは以下の新規ファイルのみです。
...
上記のファイルは、**設定ファイルと同じ場所の `output/` フォルダ**に出力されます。フォルダが無ければ作成されます。実行のたびにファイルが増えるため、元の Excel や設定ファイルに混ざらないよう分けています。
...
├── タスク管理表.xlsx
└── output/                       ← ここに出力される
    ├── run_20260831_143000.csv
    └── preview_20260831_142500_タスク管理表.md
```

変更したい場合は設定ファイルに書くか、`--output-dir` を指定します。

```yaml
output_dir: logs        # config.yaml のある場所からの相対パス。絶対パスも可
```

```bash
excel-to-backlog --config config.yaml --execute --output-dir /var/log/backlog
```
...
| 列 | 扱い |
|---|---|
| `required_cols`（判定に使う列） | 先頭行の値を保つ。同じ値の前提のため連結しない |
| 件名・日付・担当者・ステータス・カスタム属性・`key_col` | 先頭行の値を保つ。値があれば警告して無視 |
| **上記以外の列** | **空でない値だけを空行で連結する。すでに同じ値があれば連結しない** |

**すでに同じ値がある列は、重複して連結しません。** 判定に使う列（`required_cols`）以外にも、1件を通して同じ値を振っている列があるためです。
...
`枝番` を `required_cols` に入れていなくても重複しません。どの列が繰り返しなのかは表ごとに異なり、設定漏れに気づきにくいため、値が同じかどうかで判断しています。件名テンプレートに `{{項番}}-{{枝番}}` のように使っても `1-1` となり、既存課題と照合できます。

**継続行の値が空の列には、区切りを入れません。** 無条件に区切りを挟むと、2 行目以降に値が無い列で空行だけが増えていきます。
...
セル内の前後の空白・改行も除いてから連結するため、3 連続以上の改行にはなりません。

`rich_text: true` を併用している場合も同じように連結されます（取り消し線の `~~` は保たれます）。継続行かどうかの判定は書式を反映する前の値で行うため、取り消し線の有無で結合の区切りが変わることはありません。
