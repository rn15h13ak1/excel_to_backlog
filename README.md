...
同じ列名が複数あります。行データは列名をキーにするため、**左端の列だけが読み込まれ、右側の同名列は無視されます**。

ヘッダー名は Excel の表記をそのまま使い、連番などは付けません（Excel と見比べたときに名前が食い違うと混乱するため）。どの列が無視されるかは列記号付きで警告されます。

```
⚠ 同名のヘッダーが 1 件あります。左端の列だけを使い、右側の列は読み込みません:
    「備考」: B列を使用 / D列は無視
```

右側の列も使いたい場合は、Excel 側でヘッダー名を変えてください。読み込み範囲を `col_start` / `col_end` で絞る方法もあります。
